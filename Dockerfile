...
ARG OPENRESTY_NGINX_VERSION=1.19.9
...
ARG NGX_HTTP_PROXY_CONNECT_MODULE_URL=https://github.com/chobits/ngx_http_proxy_connect_module/archive/refs/tags/v${NGX_HTTP_PROXY_CONNECT_MODULE_VERSION}.tar.gz
...
RUN \
    ...
    cd openresty && \
    patch -d bundle/nginx-${OPENRESTY_NGINX_VERSION}/ -p 1 < ./../ngx_http_proxy_connect_module/patch/proxy_connect_rewrite_101504.patch && \
    cd .. && \
...
RUN \
    ...
    CARGO_HOME=/tmp/.cargo pip3 install --no-cache-dir --prefix=/usr certbot && \
